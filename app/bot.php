...
class Bot
{
    ...
    public function mtproto()
    {
        ...
        $p      = getenv('TGPORT');
        $ip     = $this->getPacConf()['domain'] ?: $this->ip;
        $st     = $this->ssh('pgrep mtproto-proxy', 'tg') ? 'включен' : 'выключен';
        $text[] = "Меню - MTProto\n";
        $text[] = "Статус: $st\n";
        if ($st == 'включен') {
    ...
    public function timer(int $client, $page)
    {
        $r = $this->send(
            $this->input['chat'],
            "Укажите дату окончания действия VPN клиента:",
            $this->input['message_id'],
            reply: 'Укажите дату окончания действия VPN клиента:',
        );
        $_SESSION['reply'][$r['result']['message_id']] = [
            'start_message'  => $this->input['message_id'],
            'start_callback' => $this->input['callback_id'],
    ...
    public function checkVersion()
    {
        try {
            ...
            if (!empty($c['admin']) && (empty($this->time) || ((time() - $this->time) > 3600))) {
                ...
                $last       = file_get_contents('https://raw.githubusercontent.com/JoricJim/SnakeGuardBot/main/version');
    ...
    public function import()
    {
        $r = $this->send(
            ...
            $this->input['message_id'],
            reply: 'Отправьте файл конфигурации:',
        );
        $_SESSION['reply'][$r['result']['message_id']] = [
            'start_message'  => $this->input['message_id'],
            'start_callback' => $this->input['callback_id'],
    ...
    public function enterPage()
    {
        $r = $this->send(
            $this->input['chat'],
            "Введите лимит страниц:",
            $this->input['message_id'],
            reply: 'Введите лимит страниц:',
        );
        $_SESSION['reply'][$r['result']['message_id']] = [
            'start_message' => $this->input['message_id'],
            'callback'      => 'setPage',
    ...
    public function include(int $count)
    {
        $r = $this->send(
            $this->input['chat'],
            "Перечислите домены через запятую:",
            $this->input['message_id'],
            reply: 'Перечислите домены через запятую:',
        );
        $_SESSION['reply'][$r['result']['message_id']] = [
            'start_message' => $this->input['message_id'],
            'callback'      => 'addInclude',
    ...
    public function defaultDNS($page = 0)
    {
        $r = $this->send(
            $this->input['chat'],
            "Введите свои DNS сервера:",
            $this->input['message_id'],
            reply: 'Введите свои DNS сервера:',
        );
        $_SESSION['reply'][$r['result']['message_id']] = [
            'start_message'  => $this->input['message_id'],
            'start_callback' => $this->input['callback_id'],
    ...
    public function changeAllowedIps($k, $page = 0)
    {
        ...
        $text    = "Меню - WireGuard - $name - Изменить IP\n\n";
        $data[]  = [
            [
    ...
    public function pacList($type, int $page, int $count = 5)
    {
        ...
        if (!empty($domains)) {
            ...
            foreach ($domains as $k => $v) {
                $data[] = [
                    [
                        'text'          => '' . ($v ? '' : '') . '' . ($type == 'includelist' ? idn_to_utf8($k) : $k),
    ...
    public function menuSS()
    {
        ...
        $ss_link = preg_replace('~==~', '', 'ss://' . base64_encode("{$ss['method']}:{$ss['password']}")) . "@$domain:$port" . (!empty($ss['plugin']) ? '?plugin=' . urlencode("v2ray-plugin;path=/v2ray;host=$domain" . (!empty($ssl) ? ';tls' : '')) : '');
        $text .= "\n\nАдрес ShadowSocks сервера:\n<code>$ss_link</code>\n";
        $text .= "\nПароль: <code>{$ss['password']}</code>";
        $text .= "\nАдрес сервера: <code>$domain:$port</code>";
        $text .= "\nШифрование:: <code>{$ss['method']}</code>";
        $text .= "\nАдрес: <code>10.10.0.5</code>";
        if ($ss['plugin']) {
            $text .= "\nПлагин: <code>v2ray-plugin</code>";
            $text .= "\nОпции: <code>$options</code>";
        }
        $data[] = [
            [
                'text'          => "V2Ray: $v2ray",
    ...
    public function xray()
    {
        ...
        $pac    = $this->getPacConf();
        $domain = $pac['domain'] ?: $this->ip;
        $st     = $this->ssh('pgrep xray', 'xr') ? 'включен' : 'выключен';
        $text[] = "Меню - XTLS\n";
        $text[] = "UUID: <code>{$c['inbounds'][0]['settings']['clients'][0]['id']}</code>";
        $text[] = "ID: <code>{$c['inbounds'][0]['streamSettings']['realitySettings']['shortIds'][0]}</code>";
        $text[] = "Публичный ключ: <code>{$pac['xray']}</code>";
        $text[] = "Фейковый домен: <code>{$c['inbounds'][0]['streamSettings']['realitySettings']['serverNames'][0]}</code>";
    ...
    {
        $text = "Меню - WireGuard - Добавить клиента\n\n";
        $data[] = [
            [
                'text'          =>  $this->i18n('all traffic'),
                'callback_data' => "/add",
    ...
    public function adguardMenu()
    {
        ...
        $domain = $conf['domain'] ?: $ip;
        $scheme = empty($ssl = $this->nginxGetTypeCert()) ? 'http' : 'https';
        $text = "Настройка AdGuard\n\n";
        if ($ssl) {
            $text .= "DNS over HTTPS:\nIP адрес: <code>$ip</code>\nАдрес: <code>$scheme://$domain/dns-query</code>\n\n";
            $text .= "DNS over TLS:\nАдрес: <code>tls://$domain</code>";
        }
        $data = [
            [
                [
    ...
    public function backup()
    {
        $r = $this->send(
            $this->input['chat'],
            "Введите кол-во бэкапов:",
            $this->input['message_id'],
            reply: 'Введите кол-во бэкапов:',
        );
        $_SESSION['reply'][$r['result']['message_id']] = [
            'start_message'  => $this->input['message_id'],
            'start_callback' => $this->input['callback_id'],
